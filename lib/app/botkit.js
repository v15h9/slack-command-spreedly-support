...
Bot.prototype.botinit = function() {
  ...
  this.userbot.on(['ambient','direct_message','direct_mention','mention'],function(bot,message) {
    /*witai.wit.runActions(
        [message.channel, message.user].join('_'), // the user's current session
        ...
        console.log(context);
        bot.reply(message, JSON.stringify(context))
      ...
      witai.wit.message(message.text, {})
      .then((data) => {
        bot.reply(message, 'Yay, got Wit.ai response: ' + JSON.stringify(data));
      })
      .catch(console.error);
  })
}
