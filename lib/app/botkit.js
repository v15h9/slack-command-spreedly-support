...
Bot.prototype.botinit = function() {
  ...
  this.userbot.on(['ambient','direct_message','direct_mention','mention'],function(bot,message) {
    ...
    witai.wit.converse([message.channel, message.user, message.ts].join('_'),message.text.substring(0, 255))
    .then((data) => {
      // Not responding to Labs-support and product-q-and-a channels
      if(['C0PG6BCQN', 'C03PGGE90'].indexOf(message.channel) === -1) {
