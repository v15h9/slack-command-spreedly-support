...
App.prototype.getSlackResponse = function(req, res) {
  return new Promise(function(resolve, reject) {
    ...
    if(text !== 'undefined' && text != '') {
      ...
      switch (message[0]) {
        case 'transaction':
          func = this.spreedlyClient.showTransaction
          break
        default:
          let err = 'Token unrecognized'
      }
      if(func !== 'undefined'){
        func('2xRrOcH2igf24wDidIb9Ohw9sQF', function(err, data){
          let messageToSend;
          if(err){
            messageToSend = err
          } else {
          ...
          bot.replyPrivateDelayed(
              {channel: req.query.channel_id, response_url: req.query.response_url},
              messageToSend,
              function(err){
                console.log(err)
              }
