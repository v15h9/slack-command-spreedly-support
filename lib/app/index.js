                ...
                "attachments": [
                  {
                    "fallback": JSON.stringify(data),
                    "color": "#36a64f",
                    "pretext": "Spreedly response for gateway token: " + message[1]
                  },
                  {
                    "fields" : app.responseFormatter(data)
                  }
        ...
        case 'payment-method':
          this.spreedlyClient.showPaymentMethod(message[1], function(err, data){
            ...
            if(err){
              ...
              messageToSend = app.responseFormatter(data)
            }
...
App.prototype.responseFormatter = function(data, type) {
  let message = {'main':[]}
  for(let key in data) {
      let val = data[key]
      if(val !== null && val.constructor === Object) {
        ...
        for(let subKey in val) {
          message[key].push({'title': subKey, 'value': val[subKey], 'short': (val[subKey] < 30)})
        }
        continue
      } else if(val !== null || val.constructor === Array) {
        val = JSON.stringify(val)
      }
      message['main'].push({'title': key, 'value': val, 'short': (val.length < 30) })
  }
  let messageToSend = {'attachments':[]}
  for(let key in message){
    messageToSend['attachments'].push({'title': (key === 'main') ? 'Spreedly response for ' + type : key, 'fields': message[key]})
  }
